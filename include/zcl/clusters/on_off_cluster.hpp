...
namespace zcl::on_off_cluster
{
  ...
  // Command descriptors
  //////////////////////////////
  const command_descriptor_t setOffCmdDescriptor{
    /*id=*/0,
    /*is_common=*/false,
    /*is_mandatory=*/true,
    /*description=*/"setOff"
  ...
  const Command setOffCommand{
    /*executer=*/setOffExecuter
  };

  const command_descriptor_t setOnCmdDescriptor{
    /*id=*/1,
    /*is_common=*/false,
    /*is_mandatory=*/true,
    /*description=*/"setOn"
  ...
  const Command setOnCommand{
    /*executer=*/setOnExecuter
  };

  const command_descriptor_t toggleCmdDescriptor{
    /*id=*/2,
    /*is_common=*/false,
    /*is_mandatory=*/true,
    /*description=*/"toggle"
  ...
  ZclStatus offWithEffectExecuter(Cluster &cluster, uint8_t effectIdentifier, uint16_t effectVariant);
  const Command offWithEffectCommand{
    /*executer=*/offWithEffectExecuter
  };
  ...
  ZclStatus onWithTimedOffExecuter(Cluster &cluster, uint8_t onOffControl, uint16_t onTime, uint16_t offWaitTime);
  const Command onWithTimedOffCommand{
    /*executer=*/onWithTimedOffExecuter
  };

  // Cluster descriptor
  const cluster_descriptor_t cluster_descriptor{
    /*id=*/6,
    /*is_msp=*/false,
    /*description=*/"onOff"
  };
  ...
  public:
    OnOffCluster()
      : Cluster(
          ...
          /*attribute_descriptors=*/{onOffAttributeDescriptor, onTimeAttributeDescriptor, offWaitTimeAttributeDescriptor},
          /*commands_map=*/{
